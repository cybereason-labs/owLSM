...
Then I find the event in output in "30" seconds:
    ...
    | data.target.process.cmd                                            | /usr/bin/ls      |
    | data.target.process.stdio_file_descriptors_at_process_creation.stdin  | FIFO             |
    | data.target.process.stdio_file_descriptors_at_process_creation.stdout | REGULAR_FILE     |
    | data.target.process.stdio_file_descriptors_at_process_creation.stderr | SOCKET           |
