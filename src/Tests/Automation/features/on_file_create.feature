...
Scenario: allowed_file_create
    ...
    Then I find the event in output in "10" seconds:
        ...
        | data.file.path           | /tmp/abc 123 *() aaa T^T^T^Y @@@ **** * * __ _ aaaaaaa.txt4.bjj6  |
        | data.file.filename       | abc 123 *() aaa T^T^T^Y @@@ **** * * __ _ aaaaaaa.txt4.bjj6       |
        | data.file.mode           | 292                                                               |
...
Scenario: blocked_file_create
    ...
    Then I find the event in output in "10" seconds:
        ...
        | type                     | FILE_CREATE                                                       |
        | process.file.path        | <automation_binary_path>                                          |
        | data.file.path           | /tmp/g . ! . # . $. ^ . _ aaa 4 5 6 999                           |
        | data.file.filename       | g . ! . # . $. ^ . _ aaa 4 5 6 999                                |
        | data.file.mode           | 292                                                               |
        | data.file.type           | REGULAR_FILE                                                      |
        | matched_rule_id          | 9                                                                 |
...
Scenario: excluded_file_create
    ...
    Then I dont find the event in output in "10" seconds:
        | type                     | FILE_CREATE                                                       |
        | data.file.path           | /tmp/test_exclude_event                                           |