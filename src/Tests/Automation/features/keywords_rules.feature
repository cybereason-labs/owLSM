...
Scenario: keyword_with_all_modifier
    ...
    Then I find the event in output in "20" seconds:
        ...
        | data.target.file.path | /tmp/test_keywords_all_modifier |
        | matched_rule_id       | 24                              |
...
Scenario: multi_keyword_rule
    ...
    Then I find the event in output in "20" seconds:
        ...
        | type                  | CHMOD                       |
        | process.file.path     | /usr/bin/chmod              |
        | data.target.file.path | /tmp/test_keywords_modifier |
        | matched_rule_id       | 25                          |
...
Scenario: multi_event_keyword_rule
    ...
    And I find the event in output in "10" seconds:
        ...
        | data.rename.destination_file.path | /tmp/multi_event_keyword_rename_dst |
        | matched_rule_id                   | 26                                  |
    And I find the event in output in "10" seconds:
        | action                        | ALLOW_EVENT                                            |
        | type                          | EXEC                                                   |
        | data.target.process.file.path | /usr/bin/echo                                          |
        | data.target.process.cmd       | /usr/bin/echo multi_event_keyword_exec_cmd please pass |
        | matched_rule_id               | 26                                                     |
    And I find the event in output in "10" seconds:
        | action                | ALLOW_EVENT                         |
        | type                  | UNLINK                              |
        | data.target.file.path | /tmp/multi_event_keyword_rename_dst |
        | matched_rule_id       | 0                                   |
