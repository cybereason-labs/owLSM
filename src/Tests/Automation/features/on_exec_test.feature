...
Scenario: allowed_exec
    ...
    Then I find the event in output in "30" seconds:
        ...
        | data.new_process.cmd           | /usr/bin/ls -l /tmp            |
...
Scenario: blocked_exec
    ...
    Then I find the event in output in "30" seconds:
        ...
        | process.file.path          | <automation_binary_path>       |
        | data.new_process.file.path | /usr/bin/ls                    |
        | data.new_process.cmd       | /usr/bin/ls -lAa /tmp          |
        | matched_rule_id            | 8                              |
...
Scenario: complex_exec_rule__match_one_event_dont_match_other_events_due_to_not_and_condition
    ...
    Then I find the event in output in "30" seconds:
        ...
        | data.new_process.cmd       | /usr/bin/echo complex_exec_test  |
        | matched_rule_id            | 2                                |
    And I find the event in output in "10" seconds:
        ...
        | process.file.path          | <automation_binary_path>         |
        | data.new_process.file.path | /usr/bin/echo                        |
        | data.new_process.cmd       | /usr/bin/echo complex_exec_excluded  |
        | matched_rule_id            | 0                                |
    And I find the event in output in "10" seconds:
        | action                     | ALLOW_EVENT                      |
        | type                       | EXEC                             |
        | process.file.path          | <automation_binary_path>         |
        | data.new_process.file.path | /usr/bin/echo                        |
        | data.new_process.cmd       | /usr/bin/echo matching_only_2_of     |
        | matched_rule_id            | 0                                |
