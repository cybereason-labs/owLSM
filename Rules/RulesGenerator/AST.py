...
class Predicate:
    ...
    def __post_init__(self):
        ...
        if active_count == 0:
            raise ValueError(
                "Predicate must have exactly one value source: "
                "string_idx != -1, numerical_value != -1, or fieldref != 'FIELD_TYPE_NONE'"
            )
...
def _preprocess_fieldref_in_dict(sel_dict: dict, fieldref_comparisons: dict) -> dict:
    """Strip numeric modifiers from fieldref fields in a single selection dict.
    pySigma handles startswith/endswith natively via SigmaFieldReference flags,
    but numeric modifiers are incompatible with SigmaFieldReference,
    so we strip them and record the comparison type."""
    new_sel = {}
    for field_key, value in sel_dict.items():
        ...
        if not extra or not (set(extra) & ALLOWED_NUMERIC_MODIFIERS):
            new_sel[field_key] = value
            continue

        field_name = parts[0]
        field_type = ALL_FIELD_TYPES.get(field_name, "string")
        mod = extra[0]
        comparison = NUMERIC_MODIFIER_TO_OPERATION.get(mod,
            COMPARISON_TYPE_EXACT_MATCH if field_type == "string" else COMPARISON_TYPE_EQUAL)

        key = (field_name, value)
        if key in fieldref_comparisons:
            raise Exception(
                f"Duplicate fieldref: field '{field_name}' references '{value}' with multiple "
                f"numeric modifiers in the same selection. Only one comparison per "
